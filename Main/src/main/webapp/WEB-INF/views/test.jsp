    ...
    <div id="mapContainer">
	    ...
	    <div id="modal_addCar" class="newModal" style="display: none">
	            ...
	            <form id="form_addCar" name="form_addCar" action="/addCar">
	               <div class="modal_body">
	                  <span id="car_numInfo"class="info"><i class="fa-solid fa-circle-info"></i> 차량번호를 입력하세요.</span>
	                  <input id="carNumInput"type="text" name="car_num" required>
           ...
           <!-- 추가추가 -->
           <script>
			    // Get references to the input field and the info span
			    const carNumInput = document.getElementById('carNumInput');
			    const vehicleNumberInfo = document.getElementById('car_numInfo');
			
			    // Add an input event listener to the input field
			    carNumInput.addEventListener('input', function () {
			        // Check if the input field has a value
			        if (carNumInput.value.trim() !== '') {
			            // If a value is entered, hide the info span
			            vehicleNumberInfo.style.display = 'none';
			        } else {
			            // If no value is entered, show the info span
			            vehicleNumberInfo.style.display = 'inline'; // or 'block' if you want it to be a block-level element
			        }
			    });
			</script>
...
canvas.addEventListener('click', function (event) {
    ...
    if (bar.length > 0) {
        ...
        console.log('테스트테스트 : ',map.date[index]);
        
        if (map.date[index] === undefined){
        	
        	drivingDate.innerHTML = '데이터가 없습니다.'
        } else{
        ...
        chart.update();
		  	...
		  	// Sample data for driving and non-driving time in hours
		        ...
		        console.log('테스트테스트 : ',map.date[0]);
		       
		        if (map.date[0] === undefined){
	   		    ...
	   		    $('#stopBtn').click(function(){
	   				...
	   				clearInterval(intervalId);
			   			...
			   			function goLiveFunction() {
			   			    ...
			   			    let year = liveDate.getFullYear();
			   			    let month = liveDate.getMonth() + 1;
			   			    let date = liveDate.getDate();
			   			    ...
			   			    fetchPost2('/goLive', obj, (map) => {
			   			        ...
			   			        // map.list의 마지막 요소를 가져와서 좌표로 사용
			   			        var lastCoordinates = map.list[map.list.length - 1];
			   			        var longitude = parseFloat(lastCoordinates.longitude);
			   			        var latitude = parseFloat(lastCoordinates.latitude);
			   			        
			   			        // 기존 live 레이어를 제거
			   			        if (live && maps.getLayers().getArray().includes(live)) {
			   			            maps.removeLayer(live);
			   			        }
			   			        
			   			        live = new ol.layer.Tile({
			   			            name: 'clean_o',
			   			            source: new ol.source.TileWMS({
			   			                url: 'http://182.222.169.37:8181/geoserver/opengis/wms',
			   			                params: {
			   			                    'layers': 'opengis:Live',
			   			                    'tiled': 'true',
			   			                    'VIEWPARAMS': 'date:' + sysdate,
			   			                    '_t': new Date().getTime()  // 캐시를 회피하기 위해 추가된 파라미터
			   			                },
			   			                serverType: 'geoserver'
			   			            })
			   			        });
	            	   ...
	            	   const intervalId = setInterval(goLiveFunction, 5000);
	            		 ...
	            		 $('#goStop').click(function(){
	            			 ...
	            			 goLive.style.display ="block";
		            		   ...
		            		   if (maps.getLayers().getArray().includes(live)) {
		            		        maps.removeLayer(live);
		            		    }
